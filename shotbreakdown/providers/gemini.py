...
프레임들(첫 / 중간 / 끝 순서)을 보고 해당 샷을 상세히 분석하세요.
...

{
  ...
  "lens_mm": "wide(24mm) | normal(50mm) | tele(85mm+) 중 추정값",
  "time_of_day": "dawn | morning | day | sunset | evening | night",
  "lighting": "조명 무드 (예: bright daylight, low-key dramatic, neon, candlelight, soft warm, harsh top-down)",
  "characters": ["등장 캐릭터 (라이브러리 이름이 있으면 그대로, 없으면 외형으로 짧게 묘사)"],
  "background": "배경 (한 줄, 구체적으로: 실내 가구·소품 / 야외 풍경)",
  "props_used": ["프레임에 보이는 주요 소품"],
  "fx_used": ["프레임에 보이는 시각효과 (없으면 빈 배열)"],
  "action": "액션/연기 (한 줄, 구체적으로)",
  "fx": "특수효과 짧게 (없으면 null; fx_used 와 중복 OK)",
...
- 인물 또는 배경이 평행 이동 → TRACK / DOLLY
- 화각이 좁아지면 ZOOM IN / DOLLY IN, 넓어지면 ZOOM OUT / DOLLY OUT
- 좌우 회전 → PAN, 상하 → TILT
- 흔들림 → HANDHELD

카메라 앵글은 피사체와 카메라 높이 관계로 판정.
조명/시간대는 프레임의 빛 색·방향·콘트라스트로 추정.

모든 텍스트는 한국어로 작성. 단, enum 값(time_of_day 등)은 영문 그대로.
"""
